...
class GameScene: SKScene {
    ...
    let impact = WKInterfaceDevice.current()
    ...
    lazy var flappybird = SKSpriteNode(texture: SKTexture(imageNamed: "flappybird").then { $0.filteringMode = .nearest }).then { $0.setScale(2.5)
    ...
    lazy var playButton = SKSpriteNode(texture: SKTexture(imageNamed: "flappyplay").then { $0.filteringMode = .nearest }).then {
        $0.name = "play"
        $0.setScale(3)
        $0.position = CGPoint(x: (width / 2), y: (height / 2) - 155)
    }
    ...
    private func spawnPipes() {
        ...
        pipes.addChild(SKNode().then {
            ...
            $0.addChild(pipeDown)
            pipeDown.isPaused = false
            $0.addChild(pipeUp)
            pipeUp.isPaused = false
            $0.addChild(contactNode)
            contactNode.isPaused = false
    ...
    public func touchFigure(_ location: CGPoint!) {
        ...
        if((!already_ran || afterGameOver == true) && (!GameScene.hitButton)){
            ...
            run(SKAction.sequence([
                ...
                SKAction.run { self.playButton.setScale(2.75) },
                SKAction.wait(forDuration: 0.1),
                SKAction.run{if(self.haptics) {self.impact.play(.click)}},
                SKAction.run { self.playButton.setScale(3) },
...
//        }
    ...
    func addResultsAndButtons() {
        if canShowScore{
            ...
            addChild(playButton)
            scaleTwice(node: playButton, firstScale: 1.0, firstScaleDuration: 0.1, secondScale: 3.0, secondScaleDuration: 0.1)
            afterGameOver = true
        }
    }
